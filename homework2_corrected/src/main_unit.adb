...
procedure main_unit is
   type BagArray is array (Positive  range<>) of character_bag.Bag;
   B_Arr: BagArray(1..Ada.Command_Line.Argument_Count);
   Count_M: Natural := 0;
   
   procedure count_multiplicity(B_Bag: in out Character_Bag.Bag) is
   begin
      Count_M := Count_M + character_bag.Multiplicity_Count(character_bag.Bag(B_Bag));
   end;
   
   procedure remove_duplicates(B_Bag: in out Character_Bag.Bag) is
   begin
      character_bag.Remove_Duplicates(character_bag.Bag(B_Bag));
   end;
   
   procedure print_bag_multiplicity(B_Bag: in out Character_Bag.Bag) is
      sum: Natural := 0;
   begin 
   ...
   procedure count_for_all is new for_all(Character_Bag.Bag, Positive, BagArray, count_multiplicity);
   procedure remove_duplicates_all is new for_all(Character_Bag.Bag, Positive, BagArray, remove_duplicates);   
   procedure print_all_bags_multiplicity is new for_all(Character_Bag.Bag, Positive, BagArray, print_bag_multiplicity);
begin
