...
package body Setq is
   ...
   function "+" (X, Y: Rational) return Rational is
   begin
       return Normalize(X.Numerator * Y.Denominator + Y.Numerator * X.Denominator,  lcm(X.Denominator, Y.Denominator));
   end "+";
   ...
   function "*" (X, Y: Rational) return Rational is
   begin
      return Normalize(X.Numerator * Y.Numerator, Y.Denominator  * X.Denominator);
   end "*";
